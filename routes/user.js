...
router.post('/signin', passport.authenticate('local.signin', {failureRedirect: '/', failureFlash: true }),
...
router.post('/signup', passport.authenticate('local.signup', {failureRedirect: '/', failureFlash: true }),
  function(req, res, next) {
    req.logIn(req.user, function(err) {
      if (err) throw err;
      res.redirect(req.session.current_url);
